...
**Progress.** Page 3 landed on 2026-09-07 and closed #1, #15 and #30, plus half of #5 and #29. Four to go.
...
| 5 | Explain the 3.1/3.4 change: `'{} {}'` vs `'{0} {1}'`; and `!s` `!r` `!a` | **partly** — the three conversion flags are now on [`repr` is not `str`](01_Text_and_Bytes/repr_is_not_str/README.md) §6; the numbering half is still open | Auto-numbering and manual numbering **cannot be mixed** — `'{} {0}'.format(a, b)` raises `ValueError`. That is the fact the changelog note leaves out, and it is the one that bites. The three conversion flags belong on the `repr` page instead |

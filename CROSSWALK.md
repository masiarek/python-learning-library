...
| …with non-ASCII escaped | `ascii(x)`, `f'{x!a}'` | [`escape_default` ↗](https://masiarek.github.io/rust-learning-library/14_Strings/str_methods/str_escape_default/index.html) | — |
| A `bytes` value | [`repr(b)`](01_Text_and_Bytes/repr_is_not_str/README.md) — `b'…'`, which is also what `>>>` shows | [`escape_ascii()` ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/printing_bytes/index.html) — the same inside on 254 of 256 bytes; both quotes always escaped | `WRITE` of an `XSTRING` — hex |
...
| Find a substring | [`s.find` / `s.index`](01_Text_and_Bytes/finding_a_substring/README.md) — character offset | [`str::find` ↗](https://masiarek.github.io/rust-learning-library/14_Strings/str_methods/str_find/index.html) — **byte** offset, and `Option` rather than `-1` | `FIND ... IN`, `sy-fdpos` |
| …and what a miss looks like | [`-1` from `find`, `ValueError` from `index`](01_Text_and_Bytes/finding_a_substring/README.md) — and `-1` is a **valid index**, so an unchecked miss slices out the last character | [`None` ↗](https://masiarek.github.io/rust-learning-library/14_Strings/searching_a_string/index.html) — there is no integer to misuse until it has been unwrapped | `sy-subrc <> 0`; the offset target is a *separate* variable |
| Cut at the first / last separator | [`s.partition` / `s.rpartition`](01_Text_and_Bytes/finding_a_substring/README.md) — always a 3-tuple, so the unpack never raises; the empty **separator** field is the only sign of a miss | [`split_once` / `rsplit_once` ↗](https://masiarek.github.io/rust-learning-library/14_Strings/str_methods/str_split_once/index.html) — `Option` of the pair, and no separator field to read | `SPLIT ... AT ... INTO a b` |
| Is it in there at all? | [`sub in s`](01_Text_and_Bytes/finding_a_substring/README.md) — and on `bytes` an **`int`** is a needle: `115 in b'spam'` is `True` | [`str::contains` ↗](https://masiarek.github.io/rust-learning-library/14_Strings/str_methods/str_contains/index.html) | `CS`, which also sets `sy-fdpos` |
...
| Substitute many characters at once | [`s.translate(table)`](01_Text_and_Bytes/translate_is_a_table/README.md) — a `dict` keyed by **ordinal**; values of any length, `None` deletes | **nothing in std** — `chars().map(…)` with a `match`, or `flat_map` to expand and delete | `TRANSLATE … USING 'axbycz'` — positional pairs, **in place**, one char in and one char out |
| …and the trap it avoids | one pass over the original, so no substitution sees another's output | [`str::replace` ↗](https://masiarek.github.io/rust-learning-library/14_Strings/str_methods/str_replace/index.html) chained has the **same** bug — `"abc"` shifted by one collapses to `"ddd"` | `REPLACE ALL OCCURRENCES` chained, same hazard |
...
| A negative number in binary | [`bin(-9)` is `'-0b1001'` — a sign and a magnitude](01_Text_and_Bytes/bin_is_not_the_bits/README.md) | `{:#b}` on `-9i8` is `0b11110111` — the type's width shows through | no conversion before C23's `%b`, and macOS's `printf` still prints `%b` as a plain `b` | [A byte is eight bits ↗](https://masiarek.github.io/encodings-learning-library/01_Bits_and_Bytes/a_byte_is_eight_bits/index.html) |
...
| What a float actually stores | [`float` is IEEE 754 double](03_Numbers/float_equality_and_nan/README.md) — there is no 32-bit one | [`f32` / `f64` ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/what_a_float_stores/index.html) | `float` / `double` | [Machine numbers ↗](https://masiarek.github.io/math-learning-library/01_Precision/machine_numbers/index.html) |
| Comparing an integer with a float | [by exact value — `2**53 + 1 == float(2**53 + 1)` is `False`](03_Numbers/comparing_int_and_float/README.md) | [does not compile until you convert one side ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/comparing_two_number_types/index.html), and `9_007_199_254_740_993_i64 as f64 == 9_007_199_254_740_992.0` is `true` | the integer becomes a `double` first, so the same comparison is `1` with no cast written | [Machine numbers ↗](https://masiarek.github.io/math-learning-library/01_Precision/machine_numbers/index.html) |
| Comparing signed with unsigned | [cannot arise — an `int` has no signedness, and `-1 < 1` is `True`](03_Numbers/comparing_int_and_float/README.md) | `-1_i32 < 1_u32` does not compile | [`-1 < 1u` is **false** ↗](https://masiarek.github.io/c-learning-library/06_Numbers/comparing_signed_and_unsigned/index.html) — `-1` becomes `UINT_MAX` | — |
| Float equality and a tolerance | [`0.1 + 0.2 == 0.3` is `False`; `math.isclose` is relative, epsilon is not](03_Numbers/float_equality_and_nan/README.md) | [same bits; `0.1_f32 + 0.2 == 0.3` is `true` by rounding luck ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/comparing_numbers_claims_checked/index.html) | same bits as `double` | [Catastrophic cancellation ↗](https://masiarek.github.io/math-learning-library/01_Precision/catastrophic_cancellation/index.html) |
| NaN, and an invalid operation | [unequal to itself except inside a container; `math.sqrt(-42.0)` raises](03_Numbers/float_equality_and_nan/README.md) | `(-42.0_f64).sqrt()` is `NaN`; `f64` is not `Ord`, so `sort()` on a `Vec<f64>` does not compile | [`sqrt(-42.0)` is NaN and sets a sticky `FE_INVALID` flag ↗](https://masiarek.github.io/c-learning-library/06_Numbers/a_float_exception_is_a_flag/index.html) | — |
...
| A number with no ceiling | `int` — every one of them; `math.factorial(100)` has 158 digits | [`num::BigInt` ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/other_number_types/big_integers/index.html) — a crate; `u128` stops at 34! | none in the standard library | — |
| Integer division with a negative operand | `-7 // 2` is `-4` — `//` floors | [`-7 / 2` is `-3` — `/` truncates; `div_euclid` floors only for a positive divisor ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/other_number_types/big_integers/index.html#three-ways-to-divide-a-negative-number) | `-7 / 2` is `-3` — truncates | — |
| An exact fraction | `fractions.Fraction` | [`num::rational::Ratio` ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/other_number_types/rational_numbers/index.html) — a crate, and `Ratio<i64>` has a ceiling | none | — |
| A complex number | `complex`, with a literal: `2.1-1.2j` | [`num::complex::Complex` ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/other_number_types/complex_numbers/index.html) — a crate, and no literal | `double complex` from `<complex.h>`: `2.1 - 1.2 * I` | — |
| A decimal number | `decimal.Decimal` — 28 significant digits by default | [`rust_decimal` ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/other_number_types/decimal_numbers/index.html) — a crate, and not `num` | `_Decimal64` — GCC 13 on Linux has it, Apple clang 21 rejects it | [Machine numbers ↗](https://masiarek.github.io/math-learning-library/01_Precision/machine_numbers/index.html) — the radix is one of the four numbers |

The fourth column is the one to notice: these are the rows where the *idea* is language-independent, so the encodings library owns it (the math library, for the float rows) and all three languages are just spellings. The float-storage and overflow rows are where they genuinely differ — Python's integers do not overflow because they are not machine words, which removes a whole class of bug and a whole class of performance guarantee at the same time. The comparison rows are the same split seen from `==`: Rust makes you convert before comparing, C converts without asking, and Python compares the exact values — until arithmetic has already turned the integer into a float.

The last five rows are the types Rust's standard library leaves out. Python ships all of them, and Rust sends you to a crate for each: [Other number types ↗](https://masiarek.github.io/rust-learning-library/19_Numbers/other_number_types/index.html) runs every one beside Python. The division row is the one that changes a program's answer: code ported from Python's `//` to Rust's or C's `/` is off by one whenever exactly one operand is negative.

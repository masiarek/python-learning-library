...
- [`gets` returns nil at EOF, and reads `ARGV`'s files first ↗](https://masiarek.github.io/ruby-learning-library/13_IO_Files_and_the_System/stdin_gets_and_argf/index.html) — the Ruby side: `gets` returns the next line with its newline and `nil` at end of input
...
- [`system` returns true, false or nil ↗](https://masiarek.github.io/ruby-learning-library/13_IO_Files_and_the_System/running_other_programs/index.html) — the Ruby side: `system` returns `true` for exit 0, `false` for any other status and `nil` when the command could not be started;…

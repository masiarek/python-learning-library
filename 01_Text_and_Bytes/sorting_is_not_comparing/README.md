...
- [Bang means dangerous, question mark means predicate ↗](https://masiarek.github.io/ruby-learning-library/02_Methods_and_Arguments/bang_and_question_methods/index.html) — the Ruby side: A trailing `!` marks the more dangerous of two versions of a method
- [Define `<=>` and Comparable gives you the rest ↗](https://masiarek.github.io/ruby-learning-library/04_Collections/comparable_and_spaceship/index.html) — the Ruby side: `<=>` answers `-1`, `0`, `1` or `nil` ("these do not compare"); `include Comparable` turns that one method into `<`,…
- [`sort_by` keys once, `sort` many times, neither is stable ↗](https://masiarek.github.io/ruby-learning-library/05_Enumerable_and_Iteration/sort_stability_and_sort_by/index.html) — the Ruby side: `sort_by` computes each element's key exactly once and `sort { |a, b| … }` recomputes at every comparison, but Ruby…

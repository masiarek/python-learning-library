...
- [A string key and a symbol key are two keys ↗](https://masiarek.github.io/ruby-learning-library/16_Idioms_and_Gotchas/string_and_symbol_keys/index.html) — the Ruby side: `{"a" => 1}[:a]` is `nil` because `"a"` and `:a` are different objects of different classes, `JSON.parse` always…
- [A symbol is a name, not a string ↗](https://masiarek.github.io/ruby-learning-library/01_Objects_and_Values/symbols_are_names/index.html) — the Ruby side: A Ruby symbol is an immutable name of which exactly one object exists per spelling
- [Ruby has block comments and a data section ↗](https://masiarek.github.io/ruby-learning-library/16_Idioms_and_Gotchas/comments_and_documentation/index.html) — the Ruby side: Ruby comments with `#`, with `=begin`/`=end` blocks that must start at column 0, and with magic comments the parser…

...
`OPEN DATASET dsn FOR INPUT IN TEXT MODE ENCODING UTF-8` is the direct counterpart of `open(path, encoding="utf-8")`, and the shape of the statement makes ABAP's version of this page's argument for you: mode and encoding are two separate clauses you must both write, and [`ENCODING` is not optional in text mode ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapopen_dataset_encoding.htm). Nor is `ENCODING DEFAULT` a bet: since 7.50 it means UTF-8 on every system, which is the default PEP 686 gives `open()`. It did depend on the system once — on a non-Unicode one [7.40 still made it `NON-UNICODE` ↗](https://help.sap.com/doc/abapdocu_740_index_htm/7.40/en-US/abapopen_dataset_encoding.htm), which there converted nothing. The locale-dependent encoding survives in two other spellings: `ENCODING NON-UNICODE` is `encoding="locale"`, the dependence asked for by name, and `IN LEGACY TEXT MODE` or `IN LEGACY BINARY MODE` [with no `CODE PAGE` ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapopen_dataset_code_page.htm) is `open(path)` with no `encoding=`, the same dependence reached by leaving something out. Both take the non-Unicode code page that table `TCP0C` assigns to the current [text environment ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abentext_environment_glosry.htm), and that belongs to the internal session, not the installation: the logon language sets it and [`SET LOCALE LANGUAGE` ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapset_locale.htm) changes it. Where Python places the bet per process, ABAP places it per session, and it is no easier to notice — one program on one system can read the same file correctly for you and wrongly for a colleague who logs on in another language.

Three differences are worth carrying across. **There is no buffering argument and no flush**: `CLOSE DATASET` is the flush, and a dataset left open at the end of a dialog step is the ABAP shape of section 4's bug. **`SET DATASET … POSITION` names a byte offset**, not a cookie — so the thing Python refuses to let you do is the only thing ABAP offers, and mixing it with `IN TEXT MODE` on a UTF-8 file is how you land mid-character with no `UnicodeDecodeError` to tell you. **And there is no rename statement at all**, which is where section 6 does not transfer: the write-beside-and-rename idiom is unavailable in pure ABAP, so a config file on the application server really is rewritten in place, and the window this page warns about is one you can only close by writing to a work file and having something outside ABAP move it. Verify the position semantics against your own system before relying on them, and the code page as well: what a `NON-UNICODE` open or a `LEGACY` one with no `CODE PAGE` uses comes from a table, `TCP0C`, not from the program, which makes it configuration in a way `DEFAULT` no longer is. *(Not machine-checked — CI cannot run ABAP.)*
...
- [A pipe is not a terminal ↗](https://masiarek.github.io/encodings-learning-library/06_Terminal/pipe_is_not_a_terminal/index.html) — section 4 from the terminal's side: `isatty()` moves `line_buffering` and nothing else, and `os._exit` down a pipe loses whatever was still buffered
...
- [`File.open` with a block closes the file for you ↗](https://masiarek.github.io/ruby-learning-library/13_IO_Files_and_the_System/file_open_with_a_block/index.html) — the Ruby side: `File.open(name, mode) { |f| … }` closes the file when the block ends and evaluates to the block's value; Python's…

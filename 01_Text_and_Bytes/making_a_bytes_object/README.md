...
`xstring` is the type, and the split is one you already have: `cl_abap_conv_codepage=>create_out( )` for text → bytes is the `.encode()` half, and there is no single constructor that also allocates, so ABAP never had this overload to trip over. What does transfer is the encoding argument, and what leaving it out does. A `create_out( )` that names no code page does not inherit the system's, which would be [UTF-16 on every current system ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abensystem_codepage_glosry.htm): [`codepage` defaults to `UTF-8` ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abencl_abap_conv_codepage.htm). That puts it beside Java 18 and Node's `Buffer.from(s)` in the paragraph above, not beside Java before 18, and it is what `.encode()` does with no argument as well. So the habit of leaving the code page out carries straight over to `.encode()` and stops at this page's constructor: `bytes(s)` has no default at all, and there the call that leaves the encoding out is the call that raises. *(Not machine-checked — CI cannot run ABAP. Verify any specific code-page number against the system rather than trusting a page.)*
...
<!-- output:making_a_bytes_object_kata_py -->
*Verified output of [`making_a_bytes_object_kata_py.py`](examples/making_a_bytes_object_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*

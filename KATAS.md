...
**What it is waiting for, and it is not a date.** The first deck is `Python::Bytes`, and it wanted two pages this library did not have. [Opening a file](01_Text_and_Bytes/opening_a_file/README.md) landed on 2026-09-08 — and it hands the encoding half of `open()` to [the encodings library ↗](https://masiarek.github.io/encodings-learning-library/04_Python/opening_a_file/index.html), so a deck drawing on it should take the `mode`, buffering and `tell()`-cookie facts from here and the codec facts from there. [Standard in, standard out, and pipes](01_Text_and_Bytes/stdin_stdout_and_pipes/README.md) is still a stub, and a deck about the type boundary that cannot say what happens to your output at the far end of a pipe is teaching half of it. Its encoding questions moved to the sibling's [A pipe is not a terminal ↗](https://masiarek.github.io/encodings-learning-library/06_Terminal/pipe_is_not_a_terminal/index.html) on 2026-09-10, so the same split applies there: `BrokenPipeError` and `sys.stdout.buffer` from here once the stub graduates, the codec facts from there. It also wants a `10_Resources/` chapter to live in, which does not exist here either — and an empty folder created in advance is clutter with a permanent URL attached.

So: katas now, deck after the second of those stubs graduates. If you are reading this and both of them are marked written in [ROADMAP.md](ROADMAP.md), the deck is the next thing.

...
- [`-n` and `-p` are a loop ↗](https://masiarek.github.io/perl-learning-library/01_One_Liners/n_and_p_are_a_loop/index.html) — the Perl switches that write the loop over input lines that `python3 -c` leaves to you
...
- [The command line is part of the language ↗](https://masiarek.github.io/ruby-learning-library/14_Tooling_Testing_and_Gems/the_command_line_flags/index.html) — the Ruby side: `ruby` takes flags for running code (`-e`), checking it (`-c`), warning about it (`-w`, `-W:category`), loading…
- [Warnings go to stderr, and `-w` finds more of them ↗](https://masiarek.github.io/ruby-learning-library/08_Errors_and_Exceptions/warnings_and_dash_w/index.html) — the Ruby side: `warn` prints to stderr; `-w` sets `$VERBOSE` to `true` and makes Ruby report unused variables, redefined methods,…
- [YJIT and ZJIT are switches, not defaults ↗](https://masiarek.github.io/ruby-learning-library/11_Under_the_Hood/jits_and_the_interpreter/index.html) — the Ruby side: MRI (`RUBY_ENGINE == "ruby"`) always runs YARV bytecode through its interpreter; YJIT (since 3.1) and ZJIT (new in…
- [`ARGV` holds strings and `ENV` is not a Hash ↗](https://masiarek.github.io/ruby-learning-library/13_IO_Files_and_the_System/argv_env_and_exit_status/index.html) — the Ruby side: `ARGV` is an Array of Strings, `$0` is the script name as it was invoked, `ENV` is a Hash-like object whose values…
- [`instance_eval` moves `self`, `class_eval` moves `def` ↗](https://masiarek.github.io/ruby-learning-library/10_Metaprogramming/instance_eval_and_class_eval/index.html) — the Ruby side: Both evals run a block with `self` set to the receiver; the difference is where a `def` inside lands
- [`irb` echoes every value; a script does not ↗](https://masiarek.github.io/ruby-learning-library/14_Tooling_Testing_and_Gems/irb_and_the_repl/index.html) — the Ruby side: `irb` is a read–eval–print loop: each line's value is printed with `inspect`, `_` holds the last one, an exception…
